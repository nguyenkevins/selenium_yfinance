...
URL = 'https://finance.yahoo.com/losers/?offset=0&count=200'
...
with open('StockTopLoser.csv', mode='w', newline='') as stockloser_file:
    stockloser_writer = csv.writer(stockloser_file, skipinitialspace=True, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    stockloser_writer.writerow(["NAME", "SYMBOL", "PRICE", "CHANGE", "CHANGE%", "VOL", "VOL3MO", "M.CAP", " ", " ", "(TOP", "STOCK", "LOSER", "TODAY)"])
    for stock_elem in stock_elems:
        ...
        change = stock_elem.find("span", class_="Trsdu(0.3s) Fw(600) C($negativeColor)" )
        ...
        print()
        stockloser_writer.writerow([name.text.strip(), symbol.text.strip(), price.text.strip(), change.text.strip(), round((1-(float(price.text.strip().replace(',', ''))/(float(price.text.strip().replace(',', ''))-float(change.text.strip()))))*100*(-1), 2), volume.text.strip(), vol3.text.strip(), mcap.text.strip()])
